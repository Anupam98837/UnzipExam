{{-- resources/views/bubble-game/bubbleGameResultStandalone.blade.php --}}
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8"/>
  ...
  <title>Bubble Game Result</title>
  ...
  <style id="erStyles">
    ...
    @media (max-width: 768px){
      .er-qcard-answers{ grid-template-columns:1fr; }
    }
    .er-q-answer-block{
      border-radius:10px;
      border:1px solid var(--line-strong);
...
<body>
  <div class="er-wrap">
    <div id="resultShell"
      ...
      <div class="er-head">
        ...
        <div>
          <h1 class="er-title" id="erGameTitle">Bubble Game Result</h1>
          ...
          <div class="er-card">
            ...
            <div class="er-score-main">
              <div class="er-score-circle">
                ...
                <div class="er-score-label">Accuracy</div>
              </div>
        ...
        <div class="er-table-head">
          <div>
            ...
            <div class="er-table-sub">
              Correct order vs your tap order, with time spent.
            </div>
          </div>
        </div>
  ...
  (function () {
    ...
    function initResultPage() {
      ...
      // Bubble game answers often are arrays/objects; normalize for display
      function normaliseAnswerField(raw) {
  if (raw == null) return "";

  // ✅ if backend sends JSON string like '["A","B"]' or '{"order":[...]}'
  ...
  if (Array.isArray(raw)) {
    return raw.map(function (item) {
      ...
      return item.text || item.title || item.value || item.name || item.label || "";
    }).filter(Boolean).join(" → ");
  }

  if (typeof raw === "object") {
    // common shapes: {order:[...]} or {sequence:[...]} or {your:[...]} etc.
    var pick =
      raw.order || raw.sequence || raw.correct || raw.your || raw.tap_order || raw.tapped || raw.path;

    if (Array.isArray(pick)) return normaliseAnswerField(pick);

    if (raw.text || raw.title || raw.value) return raw.text || raw.title || raw.value;
  ...
  return String(raw);
      ...
      // Question HTML (keeps LaTeX as-is for MathJax)
      function buildQuestionHTML(raw) {
            ...
            function wait() {
              ...
              if (mj && (mj.typesetPromise || mj.typeset)) {
                if (mj.startup && mj.startup.promise) {
                  mj.startup.promise.then(function () { resolve(mj); }).catch(reject);
                } else {
                  resolve(mj);
                }
              } else {
      ...
      function typesetMath() {
        ...
        ensureMathReady()
          .then(function (mj) {
            try {
              if (mj.typesetPromise) return mj.typesetPromise([shell]);
              if (mj.typeset) mj.typeset([shell]);
            } catch (err) {
      ...
      function renderResult(payloadRaw) {
        // Supports: {success:true, data:{...}} OR direct {game/result/questions}
        var payload = payloadRaw && payloadRaw.data ? payloadRaw.data : (payloadRaw || {});
        var game    = payload.game   || payload.bubble_game || payload.quiz || {};
        var result  = payload.result || payload || {};
        var questions = payload.questions || [];

        var gameTitle = game.title || game.game_title || game.name || "Bubble Game Result";
        ...
        // Compute counts from questions
        var totalQ = Array.isArray(questions) ? questions.length : 0;

        var correctQ = 0, wrongQ = 0, skippedQ = 0;
        var totalTimeUsed = 0;

        if (Array.isArray(questions)) {
          questions.forEach(function (q) {
            var isSkipped = (q.is_skipped === 'yes') || (q.is_skipped === 1) || (q.is_skipped === true);
            var isCorrect = (q.is_correct === 'yes') || (q.is_correct === 1) || (q.is_correct === true);

            if (isSkipped) skippedQ++;
            else if (isCorrect) correctQ++;
            else wrongQ++;

            totalTimeUsed += Number(q.spent_time_sec || q.time_spent_sec || 0);
          });
        }
        ...
        if (accuracy >= 90)      chipLabel = "Excellent";
        else if (accuracy >= 75) chipLabel = "Great work";
        else if (accuracy >= 50) chipLabel = "Keep improving";
        else                     chipLabel = "Needs practice";

        scoreChipEl.innerHTML = '<i class="fa-solid fa-award"></i> ' + chipLabel;

        percentEl.textContent = String(Math.round(accuracy)) + "%";
        marksEl.textContent   = String(score);
        attemptedEl.textContent = attemptedQ + " / " + (totalQ || "-");
        timeSpentEl.textContent  = formatDuration(totalTimeUsed);

        scoreTextEl.innerHTML =
          "You attempted <strong>" + attemptedQ +
          "</strong> out of <strong>" + totalQ +
          "</strong> questions. Correct: <strong>" + correctQ +
          "</strong>, Wrong: <strong>" + wrongQ + "</strong>.";
        ...
        resultIdEl.textContent  = result.result_id || result.id || result.result_uuid || "-";
        ...
        var maxTimeSec = 0;
        questions.forEach(function (q) {
          var t = Number(q.spent_time_sec || q.time_spent_sec || 0);
        ...
        questions.forEach(function (q, idx) {
          ...
          var qNo = q.no || q.order || q.order_no || (idx + 1);

          var textRaw = q.question || q.title || q.question_title || "";
...
var correctRaw =
  ...
  q.correct_answer ||
  q.correct_sequence ||
  q.correct_order_json ||
  q.correct_answer_json ||
  q.answer_sequence ||
  q.answer_sequence_json ||
  q.solution_sequence ||
  q.solution_order ||
  q.correct ||
  "";

          var yourAns = normaliseAnswerField(yourRaw);
          var correctAns = normaliseAnswerField(correctRaw);
          ...
          var qTime = Number(q.spent_time_sec || q.time_spent_sec || 0);
          ...
          var yourDisplay = yourAns && String(yourAns).trim() !== "" ? yourAns : "—";
          var corrDisplay = correctAns && String(correctAns).trim() !== "" ? correctAns : "—";
          ...
          qMain.innerHTML = buildQuestionHTML(textRaw);
          ...
          if (descRaw) {
            ...
            qDesc.innerHTML = buildQuestionHTML(descRaw);
            qWrap.appendChild(qDesc);
          }

          card.appendChild(qWrap);

          // Answers block (Correct Order vs Your Order)
          ...
          correctLabel.textContent = "Correct order";

          var correctTextEl = document.createElement("div");
          correctTextEl.className = "er-q-answer-text";
          correctTextEl.textContent = corrDisplay; // plain text (safe)
          ...
          yourLabel.textContent = "Your order";

          var yourTextEl = document.createElement("div");
          yourTextEl.className = "er-q-answer-text";
          yourTextEl.textContent = yourDisplay; // plain text (safe)
      ...
      async function loadResult() {
        ...
        try {
          /**
           * ✅ Bubble Game Result detail API
           * Expected response shapes supported:
           * 1) { success:true, data:{ game:{...}, result:{...}, questions:[...] } }
           * 2) { success:true, game:{...}, result:{...}, questions:[...] }
           *
           * If your route differs, just change this URL:
           */
          var res = await fetch("/api/bubble-game-results/detail/" + encodeURIComponent(RESULT_ID), {
            method: "GET",
            headers: {
      ...
      if (htmlBtn) {
        htmlBtn.addEventListener("click", function () {
          // ✅ Bubble game export API (change if your route differs)
          window.open("/api/bubble-game-results/export/" + encodeURIComponent(RESULT_ID) + "?format=html", "_blank");
        });
      }
