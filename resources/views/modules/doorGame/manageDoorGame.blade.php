{{-- resources/views/modules/doorGame/manageDoorGames.blade.php --}}
@extends('pages.users.layout.structure')

@section('title','Manage Door Games')
...
/* ===== Page shell ===== */
.dg-wrap{ max-width:1280px; margin:16px auto 44px; padding:0 6px; }
  ...
  <ul class="nav nav-tabs mb-3" role="tablist">
    <li class="nav-item">
      <a class="nav-link active" data-bs-toggle="tab" href="#tab-games" role="tab" aria-selected="true">
        <i class="fa-solid fa-door-open me-2"></i>Games
      </a>
    </li>
    <li class="nav-item">
    ...
    <div class="tab-pane fade show active" id="tab-games" role="tabpanel">
      ...
      <div class="row align-items-center g-2 mb-3 mfa-toolbar panel">
        ...
        <div class="col-12 col-xl-auto ms-xl-auto d-flex justify-content-xl-end">
          <a id="btnCreate" href="/door-games/create" class="btn btn-primary">
...
(function(){
  ...
  if (btnCreate) btnCreate.setAttribute('href','/door-games/create');
  ...
  const getKey = (r)=> (r?.uuid || r?.game_uuid || r?.door_game_uuid || r?.id);
  ...
  function settingsMini(r){
    ...
    const hasGridJson = !!(r.grid_json || r.grid_json_data || r.grid_json_blob || r.grid_json_text);
    const parts = [
      `Solution: <b>${esc(sol)}</b>`,
      `Grid JSON: <b>${hasGridJson ? 'Yes' : 'No'}</b>`,
    ];
    ...
    return `
      <div class="dropdown text-end" data-bs-display="static">
        ...
        <ul class="dropdown-menu dropdown-menu-end">
          ...
          <li><a class="dropdown-item" href="/door-games/create?edit=${encodeURIComponent(key)}">
  ...
  function normalizeListResponse(json){
    ...
    // Laravel paginator: { data: { data: [...], current_page, last_page... } }
    ...
    else if (Array.isArray(json.door_games)) items = json.door_games;

    // allow pagination override if backend sends separate pagination block
  ...
  document.addEventListener('click', async (e)=>{
    ...
    if (act === 'edit'){
      location.href = `/door-games/create?edit=${encodeURIComponent(key)}`;
      return;
    }
