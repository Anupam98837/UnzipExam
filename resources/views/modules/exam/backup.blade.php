...
<head>
  ...
  <style>
    ...
    .spin{border-radius:none!important;border-color:transparent!important}
  </style>
</head>
<body>
...
const STORAGE_KEY = 'attempt_uuid:' + QUIZ_KEY;
const REVIEW_KEY = 'attempt_review:' + QUIZ_KEY;
...
/** per-question focus start time (ms) */
let startedAtMap = {};    // qid => timestamp ms

/** pending background saves to await on submit */
...
function loadReviews(){
  try{
    ...
    reviews = (obj && typeof obj === 'object') ? obj : {};
  }catch(_){
    reviews = {};
  }
}
...
async function init(){
  try{
    ...
    startTimer(
      typeof attempt.time_left_sec === 'number'
        ? attempt.time_left_sec
        : (attempt.total_time_sec ?? 0)
    );
    ...
    renderQuestion();
    focusQuestionNoWait(questions[0]?.question_id);
...
function renderQuestion(){
  ...
  // Local-only input handlers (no await)
  if (rawType === 'fill_in_the_blank') {
...
/* ================== Instant Navigation Core ================== */
/**
 * Navigate instantly:
 * 1) Save previous question in background
 * 2) Switch UI immediately
 * 3) Focus new question in background
 */
function navigateTo(targetIdx){
  ...
  // focus new (no wait)
  focusQuestionNoWait(nextQ?.question_id);

  // background save previous (no wait)
  if (prevQ?.question_id) {
    saveQuestionNoWait(prevQ.question_id);
  }

  // unlock quickly (no network await)
  navLock = false;
}

/* ================== Focus + Save (Background) ================== */
function focusQuestionNoWait(questionId){
  if (!questionId) return;

  // mark local start time for time_spent calculation
  startedAtMap[Number(questionId)] = Date.now();

  // fire-and-forget API focus
  (async () => {
    try{
...
function calcTimeSpentSec(qid){
  const startMs = startedAtMap[Number(qid)] || null;
  if (!startMs) return 0;
  const diff = Date.now() - startMs;
  return Math.max(1, Math.round(diff / 1000));
}

function saveQuestionNoWait(questionId){
  const qid = Number(questionId);
  const q = questions.find(x => Number(x.question_id) === qid);
  if (!q) return;

  const selected = selections[qid] ?? null;

  // only count if we have start time for that question
  const timeSpent = calcTimeSpentSec(qid);

  // once saved, clear start time
  delete startedAtMap[qid];
...
function onPrev(){
  ...
  try{
    ...
    currentIndex = target;
    renderQuestion();
    focusQuestionNoWait(questions[currentIndex]?.question_id);

    // background save previous
    if (prevQ?.question_id) saveQuestionNoWait(prevQ.question_id);
  } finally {
...
function showSubmitConfirmLoading(){
  Swal.fire({
    title: 'Submitting…',
    html: 'Please wait, saving your answers.',
...
function closeSubmitConfirmLoading(){
  Swal.close();
}
...
async function doSubmit(auto){
  ...
  if (!auto) {
    const res = await Swal.fire({
      ...
      preConfirm: () => {
        // Show loading immediately on confirm click (inside modal)
        Swal.showLoading();
    ...
    // Optional: keep a separate full-screen loading modal after confirm
    // (comment this out if you only want loading inside confirm modal)
    showSubmitConfirmLoading();
  }

  try{
    isSubmitting = true;

    // lock main submit button too
    ...
    if (curQ?.question_id) saveQuestionNoWait(curQ.question_id);
    ...
    if (Swal.isVisible()) closeSubmitConfirmLoading();
