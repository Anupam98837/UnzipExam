{{-- resources/views/modules/interviewRegistrationCampaigns/manageInterviewRegistrationCampaigns.blade.php --}}

@section('title','Manage Interview Registration Campaigns')
...
<style>
/* ===== Shell ===== */
.irc-wrap{max-width:1140px;margin:16px auto 40px;overflow:visible}
  ...
  <ul class="nav nav-tabs mb-3" role="tablist">
    <li class="nav-item">
      <a class="nav-link active" data-bs-toggle="tab" href="#tab-active" role="tab" aria-selected="true">
        <i class="fa-solid fa-bullhorn me-2"></i>Active Campaigns
      </a>
    </li>
    <li class="nav-item">
      <a class="nav-link" data-bs-toggle="tab" href="#tab-inactive" role="tab" aria-selected="false">
        <i class="fa-solid fa-clock me-2"></i>Inactive Campaigns
      </a>
    </li>
  </ul>
      ...
      <div class="row align-items-center g-2 mb-3 mfa-toolbar panel">
        <div class="col-12 col-xl d-flex align-items-center flex-wrap gap-2">
          ...
          <div class="position-relative" style="min-width:300px;">
            <input id="q" type="text" class="form-control ps-5" placeholder="Search by campaign title…">
        ...
        <div class="col-12 col-xl-auto ms-xl-auto d-flex justify-content-xl-end">
          <a id="btnCreate" href="/interview-registration-campaigns/create" class="btn btn-primary">
          ...
          <div class="table-responsive">
            <table class="table table-hover table-borderless align-middle mb-0">
              <thead class="sticky-top">
                <tr>
                  <th class="sortable" data-col="title">CAMPAIGN <span class="caret"></span></th>
                  <th style="width:220px;">FOLDER</th>
                  <th style="width:240px;">DATE RANGE</th>
                  <th style="width:280px;">REGISTER URL</th>
  ...
  <div class="modal-dialog modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title"><i class="fa fa-filter me-2"></i>Filter Campaigns</h5>
      ...
      <div class="modal-body">
        <div class="mb-3">
          ...
          <div class="text-muted small mt-1">Optional. Filter campaigns by specific folder.</div>
        </div>
      </div>
...
(function(){
  ...
  const fmtRange=(a,b)=> {
    ...
    const f = (d)=> d ? d.toLocaleDateString(undefined,{year:'numeric',month:'short',day:'2-digit'}) : '-';
    return `${f(aa)} → ${f(bb)}`;
  };
  ...
  function paramsBase(scope){
    ...
    usp.set('status', scope === 'active' ? 'active' : 'inactive');

    if (q && q.value.trim()) usp.set('q', q.value.trim());
    if (filterFolder && String(filterFolder.value||'').trim()) usp.set('user_folder_id', String(filterFolder.value).trim());

    return usp.toString();
  }

  function urlFor(scope){
    return '/api/interview-registration-campaigns?' + paramsBase(scope);
  }

  /* ========= Row builders ========= */
  function actionMenu(scope, r){
    const key  = r.id; // keep API by ID
    ...
    return `
      <div class="dropdown text-end" data-bs-display="static">
        ...
        <ul class="dropdown-menu dropdown-menu-end">
          ...
          <li>
            <button class="dropdown-item" data-act="copy" data-url="${esc(r.register_url || '')}">
  ...
  function rowHTML(scope, r){
    ...
    const folderName = esc(r.folder_title || r.folder?.title || '-');
    const range = fmtRange(r.start_date, r.end_date);
    const url = esc(r.register_url || (r.uuid ? (`${location.origin}/register/${r.uuid}`) : '-'));
  ...
  async function load(scope){
    ...
    try{
      ...
      const pagination = json?.meta || {current_page:1, per_page:20, total:items.length, total_pages:1};
  ...
  async function loadFoldersForFilter(){
    try{
      ...
      const opt = folders.map(f => `<option value="${f.id}">${esc(f.title || ('Folder #'+f.id))}</option>`).join('');
      filterFolder.insertAdjacentHTML('beforeend', opt);
    }catch(ex){
      console.error(ex);
    }
  }
  ...
  document.addEventListener('click', async (e)=>{
    ...
    const name = it.dataset.name || 'this campaign';

    if (act === 'edit'){
      location.href = `/interview-registration-campaigns/create?edit=${encodeURIComponent(key)}`;
      return;
    }
    ...
    if (act === 'activate'){
      ...
      await updateCampaignStatus(key, 'active', 'Campaign activated');
    ...
    if (act === 'deactivate'){
      ...
      await updateCampaignStatus(key, 'inactive', 'Campaign deactivated');
      load('active'); load('inactive');
      return;
    }
    ...
    if (act === 'delete'){
      const {isConfirmed} = await Swal.fire({
        ...
        html:`Campaign "${esc(name)}" will be removed (soft delete).`,
        showCancelButton:true,
      ...
      try{
        const res = await fetch(`/api/interview-registration-campaigns/${encodeURIComponent(key)}`, {
          method:'DELETE',
          headers:{ 'Authorization':'Bearer '+TOKEN, 'Accept':'application/json' }
        });
        const j = await res.json().catch(()=> ({}));
  ...
  async function updateCampaignStatus(id, statusVal, doneMsg){
    try{
      const res = await fetch(`/api/interview-registration-campaigns/${encodeURIComponent(id)}`, {
        method: 'PUT',
        headers:{
          'Authorization':'Bearer '+TOKEN,
