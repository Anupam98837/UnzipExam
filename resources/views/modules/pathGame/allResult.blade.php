{{-- resources/views/modules/path_game/managePathGameResults.blade.php --}}
@extends('pages.users.layout.structure')

@section('title','Path Game Results')
              ...
              <table class="table table-hover table-borderless align-middle mb-0" id="tbl-results">
                <thead class="sticky-top">
                  <tr>
                    ...
                    <th class="sortable" data-col="game_title">PATH GAME <span class="caret"></span></th>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="row g-2">
          ...
          <div class="col-12">
            <label class="form-label">Path Game (Select)</label>
...
(function(){
  ...
  // ✅ CHANGE THESE IF YOUR ROUTES DIFFER
    const PATH_RESULTS_ACTION_API = '/api/path-game-results';

  const PATH_RESULTS_LIST_API   = '/api/path-game-results';
  const PATH_GAMES_LIST_API     = '/api/path-games?per_page=100&status=active';
  const PATH_RESULTS_EXPORT_API = '/api/path-game/result/export';

  // ✅ Folder list API
  const PATH_FOLDERS_LIST_API   = '/api/user-folders';
  ...
  async function exportResults(){
    ...
    const url = `${PATH_RESULTS_EXPORT_API}?${usp.toString()}`;
    ...
    try{
      ...
      let filename = `path-game-results-${scope}.csv`;
  ...
  async function patchPublishAny(kind, payload){
    ...
    if (kind === 'single'){
      ...
      url = toPublish
        ? `${PATH_RESULTS_ACTION_API}/${encodeURIComponent(uuid)}/publish-to-student`
        : `${PATH_RESULTS_ACTION_API}/${encodeURIComponent(uuid)}/unpublish-to-student`;
    ...
    if (kind === 'bulk'){
      ...
      url = `${PATH_RESULTS_ACTION_API}/bulk/publish-any`;
  ...
  function actionMenu(item){
    ...
    // ✅ Adjust this view route if your project uses a different URL
    const viewUrl = rid ? `/path-game/results/${encodeURIComponent(rid)}/view` : '#';
  ...
  function syncBulkUI(){
    ...
    loaderColResults.setAttribute('colspan', isOn ? '11' : '10');
  ...
  function pickPct(result){
    const r = result || {};
    const val = (r.accuracy ?? r.percentage ?? r.pct ?? null);
  ...
  function buildRowResults(item){
    ...
    tdStatus.innerHTML = statusBadge(attempt.status || result.status);
  ...
  async function loadGamesForFilter(){
    try{
      const res = await fetch(PATH_GAMES_LIST_API, {
  ...
  async function loadFoldersForFilter(){
    try{
      const res = await fetch(PATH_FOLDERS_LIST_API, {
        headers: { 'Authorization':'Bearer '+TOKEN, 'Accept':'application/json' }
      });
      const json = await res.json().catch(()=> ({}));
  ...
  function buildParams(scope){
    ...
    if (fGameId && fGameId.value) usp.set('path_game_id', fGameId.value);

    // ✅ Folder filter (supports both keys)
  ...
  function urlFor(scope){
    return `${PATH_RESULTS_LIST_API}?${buildParams(scope)}`;
  }
