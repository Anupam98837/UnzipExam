{{-- resources/views/modules/pathGame/managePathGames.blade.php --}}
@extends('pages.users.layout.structure')

@section('title','Manage Path Games')
...
<style>
/* ===== COMPLETE DROPDOWN FIX (same as Quiz/Bubble page) ===== */
.pg-wrap,
.table-wrap,
.card,
.card-body,
...
/* ===== Page shell ===== */
.pg-wrap{ max-width:1280px; margin:16px auto 44px; padding:0 6px; }
  ...
  <ul class="nav nav-tabs mb-3" role="tablist">
    <li class="nav-item">
      <a class="nav-link active" data-bs-toggle="tab" href="#tab-games" role="tab" aria-selected="true">
        <i class="fa-solid fa-route me-2"></i>Games
      </a>
    </li>
    <li class="nav-item">
    ...
    <div class="tab-pane fade show active" id="tab-games" role="tabpanel">
      ...
      <div class="row align-items-center g-2 mb-3 mfa-toolbar panel">
        ...
        <div class="col-12 col-xl-auto ms-xl-auto d-flex justify-content-xl-end">
          <a id="btnCreate" href="/path-games/create" class="btn btn-primary">
            ...
            <table class="table table-hover table-borderless align-middle mb-0">
              <thead class="sticky-top">
                <tr>
                  ...
                  <th class="sortable" data-col="grid_dim" style="width:120px;">GRID <span class="caret"></span></th>
                  <th class="sortable" data-col="max_attempts" style="width:140px;">MAX ATTEMPTS <span class="caret"></span></th>
                  <th class="sortable" data-col="time_limit_sec" style="width:160px;">TIME LIMIT <span class="caret"></span></th>
            ...
            <table class="table table-hover table-borderless align-middle mb-0">
              <thead class="sticky-top">
                <tr>
                  ...
                  <th style="width:160px;">TIME LIMIT</th>
                  <th style="width:220px;">SETTINGS</th>
...
(function(){
  ...
  if (btnCreate) btnCreate.setAttribute('href','/path-games/create');
  ...
  // ✅ FIX: send SIMPLE column names (backend safe + common)
  const sortMap = {
    title: 'title',
    status: 'status',
    grid_dim: 'grid_dim',
  ...
  const getKey = (r)=> (r?.uuid || r?.game_uuid || r?.path_game_uuid || r?.id);
  ...
  function paramsBase(scope){
    ...
    usp.set('order_by', sortMap[sortKey] || 'created_at');
    ...
    if (scope === 'games'){
      ...
      if (st !== null && st !== undefined && st !== '') usp.set('status', st);
      // if st == '' => no filter (all)
    }
  ...
  function settingsMini(r){
    ...
    const rotEn = (Number(r.rotation_enabled ?? 1) === 1) ? 'Yes' : 'No';
    const rotMode = String(r.rotation_mode || 'both').toLowerCase();
    const parts = [
      `Solution: <b>${esc(sol)}</b>`,
      `Rotation: <b>${esc(rotEn)}</b> (${esc(rotMode)})`,
    ];
    ...
    return `
      <div class="dropdown text-end" data-bs-display="static">
        ...
        <ul class="dropdown-menu dropdown-menu-end">
          ...
          <li><a class="dropdown-item" href="/path-games/create?edit=${encodeURIComponent(key)}">
            <i class="fa fa-table-cells"></i> Manage Grid
          </a></li>
  ...
  function rowHTML(scope, r){
    ...
    const gridDim = Number(r.grid_dim ?? r.grid_dimension ?? 3);
    const maxAttempts = Number(r.max_attempts ?? r.max_attempt ?? 1);
    const timeLimit = Number(r.time_limit_sec ?? r.time_limit ?? 30);
      ...
      tr.innerHTML = `
        <td>
          <div class="fw-semibold">
            <a href="/path/tests/play?game=${encodeURIComponent(key)}" class="link-offset-2 link-underline-opacity-0">${title}</a>
          </div>
        </td>
  ...
  // ✅ FIX: Normalize API responses (array OR paginator)
  function normalizeListResponse(json){
  let items = [];
  let pg = {};

  if (!json || typeof json !== 'object') return { items, pg };

  // ✅ YOUR API FORMAT: { data: { items: [...], pagination: {...} } }
  if (json.data && typeof json.data === 'object') {
    if (Array.isArray(json.data.items)) {
      items = json.data.items;

      if (json.data.pagination && typeof json.data.pagination === 'object') {
        pg = {
          total: json.data.pagination.total ?? items.length,
          per_page: json.data.pagination.per_page ?? items.length,
          current_page: json.data.pagination.current_page ?? 1,
          last_page: json.data.pagination.last_page ?? 1,
        };

        // optional
        if ('from' in json.data.pagination) pg.from = json.data.pagination.from;
        if ('to' in json.data.pagination) pg.to = json.data.pagination.to;
      }

      return { items, pg };
    }

    // ✅ Laravel paginator support: { data: { data: [...] } }
  ...
  else if (Array.isArray(json.games)) items = json.games;
  else if (Array.isArray(json.path_games)) items = json.path_games;
  else if (Array.isArray(json.data)) items = json.data;

  if (json.pagination && typeof json.pagination === 'object') {
    pg = Object.assign(pg, json.pagination);
  }
  ...
  async function load(scope){
    ...
    try{
      ...
      const { items, pg } = normalizeListResponse(json);
      ...
      const perPage    = Number(pg.per_page ?? Number(perPageSel?.value || 20) ?? 20);
      const current    = Number(pg.current_page ?? state[scope].page ?? 1);
      ...
      if (meta){
        const from = pg.from ?? (total ? ((current-1)*perPage + 1) : 0);
  ...
  document.addEventListener('click', async (e)=>{
    ...
    if (act === 'edit'){
      location.href = `/path-games/create?edit=${encodeURIComponent(key)}`;
      return;
    }
