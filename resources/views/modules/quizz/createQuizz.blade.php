  ...
  <div class="card qz">
    <div class="card-header">
      <div class="qz-head">
        ...
        <strong id="pageTitle">Create Quiz</strong>
        <span class="hint" id="hint">— Fill title, description, instructions, image & schedule.</span>
      </div>
    </div>
...
(function(){
  ...
  async function loadQuiz(key){
    ...
    try{
      ...
      // Handle image preview if exists (quiz_img column)
      if(q.quiz_img){
        const srcRaw = String(q.quiz_img);
        const src = srcRaw.startsWith('http')
          ? srcRaw
          : ('/' + srcRaw.replace(/^\/+/, ''));
        // prefer URL tab for editing existing URL/path
        urlInput.value = src;
        urlPrev.style.backgroundImage = `url('${src}')`;
        const urlTabTrigger = document.querySelector('#url-tab');
        if(urlTabTrigger){
  ...
  $('btnSave').addEventListener('click', async ()=>{
    ...
    if(!name){
      fErr('quiz_name','Quiz title is required.');
      $('quiz_name').focus();
      return;
    }
