...
<div class="qz-wrap">
  ...
  <ul class="nav nav-tabs mb-3" role="tablist">
    <li class="nav-item">
      <a class="nav-link active" data-bs-toggle="tab" href="#tab-quizzes" role="tab" aria-selected="true">
        <i class="fa-solid fa-layer-group me-2"></i>Quizzes
      </a>
    </li>
    <li class="nav-item">
  ...
  <div class="modal-dialog modal-lg modal-dialog-scrollable">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button id="btnAddNote" class="btn btn-primary">
...
(function(){
  ...
  function actionMenu(scope, r){
    ...
    return `
      <div class="dropdown text-end" data-bs-display="static">
        ...
        <ul class="dropdown-menu dropdown-menu-end">
          <li><button class="dropdown-item" data-act="restore" data-key="${key}" data-name="${esc(r.quiz_name||'')}">
            <i class="fa fa-rotate-left"></i> Restore
          </button></li>
          <li><button class="dropdown-item text-danger" data-act="force" data-key="${key}" data-name="${esc(r.quiz_name||'')}">
  ...
  async function load(scope){
    ...
    try{
      ...
      if (scope==='deleted'){
        items.forEach(it => it.deleted_at = it.deleted_at || it.updated_at || it.created_at);
      }
...
filterModalEl?.addEventListener('hidden.bs.modal', ()=>{
  ...
  document.addEventListener('click', async (e)=>{
    ...
    if (act==='archive'){
      ...
      await callStatus(key, 'archived', 'Quiz archived');
      load('active');
      load('archived');
      return;
    }
    if (act==='unarchive'){
      ...
      if(!isConfirmed) return;
      await callStatus(key, 'active', 'Quiz unarchived');
      load('archived');
      load('active');
      return;
    }
    ...
    if (act==='restore'){
      ...
      if(!isConfirmed) return;
      try{
        const res = await fetch(`/api/quizz/${encodeURIComponent(key)}/restore`, {
          method:'PATCH',
          headers:{'Authorization':'Bearer '+TOKEN,'Accept':'application/json'}
        });
        const j=await res.json().catch(()=>({}));
    ...
    if (act==='force'){
      ...
      if(!isConfirmed) return;
      try{
        const res = await fetch(`/api/quizz/${encodeURIComponent(key)}/force`, {
          method:'DELETE',
          headers:{'Authorization':'Bearer '+TOKEN,'Accept':'application/json'}
        });
        const j=await res.json().catch(()=>({}));
  ...
  async function saveRandomSettings(){
    ...
    try{
      ...
      setBtnBusy(btnSaveRandom, false);
    }
  }
  ...
  async function openNotes(key, name){
    ...
    notesModal.show();
    try{
      const res = await fetch(`/api/quizz/${encodeURIComponent(key)}/notes`, {
        headers:{'Authorization':'Bearer '+TOKEN,'Accept':'application/json'}
      });
      const j = await res.json().catch(()=>({}));
      ...
      if(items.length===0){
        notesList.innerHTML = '<div class="small text-muted">No notes yet.</div>';
        return;
      }
  ...
  async function addNote(){
    ...
    if(!text){
      return Swal.fire('Note required','Please type something.','info');
    }
    try{
      ...
      noteText.value='';
      ok('Note added');
      openNotes(currentKeyForNotes, 'Quiz');
    }catch(e){
      err(e.message||'Add note failed');
    }
  }
})();
</script>
