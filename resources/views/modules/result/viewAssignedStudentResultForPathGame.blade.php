{{-- resources/views/examiner/path_game/viewAssignedStudentResult.blade.php --}}
...
(function(){
  ...
  function extractGameKey(row){
    const game = (row && (row.game || row.path_game)) ? (row.game || row.path_game) : null;
  ...
  function decideInitialGameKey(rows){
    ...
    if (gameKey){
      ...
      const byKey = rows.find(r => {
        ...
        const gid = (r.path_game_id != null) ? String(r.path_game_id) : ((r.game_id != null) ? String(r.game_id) : null);
    ...
    if (urlGameParam){
      ...
      const byAssign = rows.find(r => {
        ...
        const gid  = (r.path_game_id != null) ? String(r.path_game_id) : ((r.game_id != null) ? String(r.game_id) : null);
        const gu   = (r.path_game_uuid != null) ? String(r.path_game_uuid) : ((r.game_uuid != null) ? String(r.game_uuid) : null);
  ...
  function extractRowsFromGameResponse(json){
    ...
    else if (Array.isArray(json.path_games)) rows = json.path_games;
  ...
  function normalizeFromAttempts(){
    ...
    const game    = state.raw.game || state.raw.path_game || {};
    ...
    attempts.forEach(a => {
      ...
      // ✅ try to read real percentage/accuracy from API if you send it
      const pct = (a.percentage != null) ? Number(a.percentage)
               : (a.accuracy != null) ? Number(a.accuracy)
               : null;
      ...
      if (pct != null) {
        ...
        sumPct += pct;
        countPct++;
      } else if (score != null && (!currentBest || currentBest.score == null || score > currentBest.score)) {
    ...
    studentMap.forEach(st => {
      ...
      else if (br.percentage != null || br.score != null) failCount++;
    });
  ...
  function applyFilters(){
    ...
    if (status !== 'all'){
      arr = arr.filter(st => {
        ...
        if (status === 'fail') return (best.percentage != null || best.score != null) && !best.is_pass;
    ...
    arr.sort((a,b) => {
      ...
      switch(sort){
        ...
        case 'best_desc': return (bestB.percentage || bestB.score || 0) - (bestA.percentage || bestA.score || 0);
        case 'best_asc': return (bestA.percentage || bestA.score || 0) - (bestB.percentage || bestB.score || 0);
  ...
  function renderHeader(){
    ...
    if (els.gameSub) els.gameSub.textContent = 'View student-wise performance and analytics for this path game.';
  ...
  function renderDistributionAndSnapshot(){
    ...
    attempts.forEach(a => {
      ...
      if (snap && Array.isArray(snap.path)) sumMoves += snap.path.length; // fallback
  ...
  function renderTable(){
    ...
    state.filtered.forEach((st, idx) => {
      ...
      if (best && (pct !== null && pct !== undefined || best.score != null)){
  ...
  function renderDetail(data, studentName){
    ...
    const game    = payload.game || payload.path_game || {};
    ...
    const rotations = Array.isArray(snap.rotations) ? snap.rotations : [];
    const arrows = snap.final_arrows || snap.finalArrows || snap.arrows || null;

    const status = String(result.status || '').toLowerCase();
    const score = (result.score != null) ? result.score : '—';

    // ✅ percent: prefer percentage/accuracy if sent
    const perc = (result.percentage != null) ? result.percentage
               : (result.accuracy != null) ? result.accuracy
               : null;

    const timeMs = result.time_taken_ms ?? snap?.timing?.time_taken_ms ?? null;
      ...
      <div class="asr-detail-head">
        <div>
          ...
          <div class="asr-detail-sub">
            ${(game.title || game.game_title || game.name || 'Path Game')}
            • Score: ${score}
            • Time: ${timeMs != null ? msToPretty(timeMs) : '—'}
            • Moves: ${moves.length || path.length}
    ...
    snapCard.innerHTML = `
      ...
      <div class="asr-q-body">
        ...
        <div><strong>Rotations:</strong> ${rotations.length ? rotations.length : '—'}</div>
        <div><strong>Final Arrows:</strong> ${arrows ? (typeof arrows === 'string' ? arrows : JSON.stringify(arrows)) : '—'}</div>
      </div>
    `;
    inner.appendChild(snapCard);

    // Moves
    const moveList = moves.length ? moves : (path.length ? path.map((v, i) => ({ step: i+1, at: v })) : []);
    if (!moveList.length){
      ...
      moveList.forEach((m, idx) => {
        const from = (m && m.from != null) ? m.from : (m && m.at != null ? m.at : '—');
        ...
        div.innerHTML = `
          <div class="asr-q-head">
            <div class="asr-q-title">Move #${idx+1}${to !== '—' ? `: ${from} → ${to}` : `: ${from}`}</div>
  ...
  function loadResultDetail(resultId, studentName){
    ...
    // ✅ PATH GAME detail API
    const url = `/api/path-game-results/detail/${encodeURIComponent(resultId)}`;
  ...
  function loadOverview(){
    ...
    // ✅ PATH GAME assigned results API
    const url = `/api/path-game-results/assigned/${encodeURIComponent(gameKey)}`;
  ...
  function loadGameOptions(){
    ...
    // ✅ PATH GAME list API
    const url = '/api/path-games?per_page=100&status=active';
    ...
    fetch(url, {
      ...
      .then(res => res.json().catch(() => ({})))
      .then(json => {
        console.log('Examiner path games list response:', json);
        ...
        if (!rows || !rows.length){
          ...
          if (els.tableBody){
            els.tableBody.innerHTML = `
              <tr><td colspan="7"><div class="asr-empty">No assigned path games found for your examiner account.</div></td></tr>`;
          }
        ...
        rows.forEach(row => {
          ...
          opt.textContent =
            ...
            row.game_title ||
            row.name ||
            ('Game #' + (row.path_game_id || row.game_id || row.id || ''));
          sel.appendChild(opt);
        });
